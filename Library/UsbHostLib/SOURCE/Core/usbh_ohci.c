 ...
 * $Revision: 8 $
 * $Date: 14/10/07 5:47p $
 * @brief    NUC505 MCU USB Host OHCI driver
 *
 * @note
    ...
    if (usb_pipetype(pipe) == PIPE_ISOCHRONOUS) {
        if (urb->transfer_flags & USB_ISO_ASAP) {
            urb->start_frame = ((ed->state == ED_OPER)
                                ...
                                : (ohci.hcca->frame_no) + OHCI_ISO_DELAY) & 0xffff;
        }
    }
...
static void td_submit_urb(URB_T * urb)
{
    ...
    case PIPE_ISOCHRONOUS:
        for (cnt = 0; cnt < urb->number_of_packets / ISO_FRAME_COUNT; cnt++) {
            iso_td_fill(TD_CC | ((ISO_FRAME_COUNT - 1) << 24) | TD_ISO | ((urb->start_frame + cnt * ISO_FRAME_COUNT) & 0xffff ),
...
static TD_T  *dl_reverse_done_list()
{
    ...
    while (td_list_hc) {
        ...
        if (TD_CC_GET((uint32_t)td_list->hwINFO)) {
            ...
            if ((td_list->hwPSW[0] & 0xE000) != 0xE000)
                USB_error("CC Error!! - DEV[%d] ED[%d] PSW:[%x] - ", td_list->urb->dev->devnum, (td_list->ed->hwINFO >> 7) & 0xf, td_list->hwPSW[0] >> 12);
        ...
        if ((td_list->ed->type == PIPE_ISOCHRONOUS) &&
                ...
                ((td_list->hwPSW[0] >> 12) != TD_NOTACCESSED)) {
            USB_error("PSW Error!! - TD[%x] DEV[%d] ED[%d] PSW:[%x] - ", (int)td_list, td_list->urb->dev->devnum, (td_list->ed->hwINFO >> 7) & 0xf, td_list->hwPSW[0] >> 12);
...
static int  rh_send_irq(void *rh_data, int rh_len)
{
    ...
    *(uint8_t *)data = (USBH->HcRhStatus & (RH_HS_LPSC | RH_HS_OCIC)) ? 1: 0;
    ...
    for ( i = 0; i < num_ports; i++) {
        *(uint8_t *)(data + (i + 1) / 8) |= (((i?USBH->HcRhPortStatus2:USBH->HcRhPortStatus1) &
...
#define WR_RH_PORTSTAT(x)       (wIndex-1)?(USBH->HcRhPortStatus2 = (x)):(USBH->HcRhPortStatus1 = (x))
#define RD_RH_STAT              (USBH->HcRhStatus)
#define RD_RH_PORTSTAT          (wIndex-1)?USBH->HcRhPortStatus2:USBH->HcRhPortStatus1
...
void USBH_IRQHandler(void)
{
    ...
    //printf("[irq-0x%x] 0x%x, 0x%x\n", ints, USBH->RHPSTS[0], USBH->RHPSTS[1]);
