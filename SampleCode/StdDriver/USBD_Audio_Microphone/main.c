...
void InternalCODEC_Setup(int8_t i8RecVol);
extern uint32_t g_ubsd_tx_flag;

extern volatile uint8_t bRecVolumeAdjust;
extern volatile uint8_t bSampleRateAdjust;
extern volatile uint8_t bRecMuteAdjust;

extern volatile uint32_t g_usbd_SampleRate;
extern volatile uint32_t g_usbd_SampleBitRate;
extern uint8_t  g_usbd_RecMute;
extern int16_t  g_usbd_RecVolume;
...
volatile uint32_t g_usbd_pollingclassrequest = 0;
volatile uint32_t g_usbd_pollingsetinterface = 0;
volatile uint32_t g_usbd_pollingsetinterfacealtinterface = 0;

void UAC_ClassRequest2(void)
{
	if ( g_usbd_pollingclassrequest == 0 )
		g_usbd_pollingclassrequest = 1;
}

void UAC_SetInterface2(uint32_t u32AltInterface)
{
	if ( g_usbd_pollingsetinterface == 0 )
	{
		g_usbd_pollingsetinterface = 1;
		g_usbd_pollingsetinterfacealtinterface = u32AltInterface;
	}
}

int main(void)
 {
		uint32_t u32CurSR, u32CurSBR;
...
//    UART1_Init();
    ...
    printf("NUC505 USB UAC - Microphone");
#ifdef __HID__
		printf(" + HID");
#ifdef __KEYBOARD__
		printf(" - Keyboard");
...
//        printf("Relocate vector table in SRAM (0x%08X) for fast interrupt handling.\n", Image$$ER_VECTOR2$$ZI$$Base);
		...
		I2S_Init(REC_DEF_RATE, REC_BIT_RATE);
	...
	KEY_Init();
		...
		/* FIXME: support stereo and 16-bit only for now */
		if ( REC_BIT_RATE == 16 )
		I2S_Open(I2S, I2S_MODE_MASTER, REC_DEF_RATE, I2S_DATABIT_16, I2S_STEREO, I2S_FORMAT_I2S, I2S_ENABLE_INTERNAL_CODEC);
		else
			printf( "Error! Bit rate not supported %d\n", REC_BIT_RATE );
	u32CurSR = REC_DEF_RATE;
		u32CurSBR = REC_BIT_RATE;
		I2S_EnableMCLK(I2S, REC_DEF_RATE*256);
		
		I2S_SET_RX_TH_LEVEL(I2S, I2S_FIFO_RX_LEVEL_WORD_8);
		
		InternalCODEC_Setup(g_usbd_RecVolume);
//	printf("1 %d %d %d\n", u32CurSR, g_usbd_SampleRate, bSampleRateAdjust);
  ...
  I2S_ENABLE_RXDMA(I2S);
    ...
    USBD_Open(&gsInfo, UAC_ClassRequest2, UAC_SetInterface2);
    ...
    while(1)
		{
				if(g_ubsd_tx_flag&& (USBD->EP[EPB].EPDATCNT == 0))
				{
						EPA_Handler();
						g_ubsd_tx_flag = 0;
				}
...
//				printf("2 %d %d %d\n", u32CurSR, g_usbd_SampleRate, bSampleRateAdjust);
				...
				HID_UpdateKbData();
#endif							
			if ( g_usbd_pollingclassrequest == 1 )
			{
//							if(g_volume_flag != 0)
//							{
									UAC_ClassRequest();
									g_usbd_pollingclassrequest = 0;
//							}
			}

			if ( g_usbd_pollingsetinterface == 1 )
			{
				UAC_SetInterface( g_usbd_pollingsetinterfacealtinterface );
				g_usbd_pollingsetinterface = 0;
			}
		}
}
...
/*---------------------------------------------------------------------------------------------------------*/
void demo_LineIn(int8_t i8RecVol)
{
	uint32_t i;
	...
	else
	{
		...
		I2S_SET_INTERNAL_CODEC(I2S, 0x10, i8RecVol);	//Un-Mute the ADC Left channel volume
		I2S_SET_INTERNAL_CODEC(I2S, 0x11, i8RecVol);	//Un-Mute the ADC Right channel volume
...
void demo_MIC0(int8_t i8RecVol)
{
	uint32_t i;
...
void demo_MIC1(int8_t i8RecVol)
{
	uint32_t i;
